...
export async function listBabies(familyId) {
  if (!familyId) return []
  const { data } = await supabase.db.select('babies', {
    select: '*',
...
export async function createBaby(familyId, { name, gender, birthday }) {
  const rows = await supabase.db.insert('babies', {
    family_id: familyId,
...
export async function saveBaby(baby) {
  const rows = await supabase.db.upsert('babies', {
    ...baby,
...
export async function uploadBabyAvatar(familyId, babyId, filePath) {
  ...
  await supabase.storage.uploadObject(path, filePath, { upsert: true })
  return path
}
...
export async function resolveStorageUrl(path) {
  if (!path) return ''
  return supabase.storage.createSignedUrl(path, 3600)
}
